...
\begin{itemize}
    \item \autoref{ch:background} presents the required mathematical concepts that will be used throughout the thesis
    \item \autoref{chap:modeling} presents how a general \gls{aiauv} can be modeled along with conventions usied throughout the thisis.
    \item \autoref{ch:software} presnts the software developed as part of this thisis. This includes control visualization and simulation software.
    \item \autoref{ch:experimental_setup} presents the Eelume model M robot used for experiments, along with assumptions and limitations of the system. 
    \item \autoref{ch:results} presents the results of kinematic-level \gls{tpc} experiments conducted
    \item \autoref{ch:conclusion} summarizes the work and findings, and gives suggestions for further work.
\end{itemize}
\fi

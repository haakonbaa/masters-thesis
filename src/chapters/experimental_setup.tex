...
group, a specific setpoint. The four groups are described in more detail below.

\subsection*{Lower-Level \gls{pd+} Controller}
The first group of experiments aims to show that the lower-level \gls{pd+} controller
is working as intended. This means that the controller is able to follow a setpoint
...
\subsection*{Velocity-Level Task-Priority Controller}
...
and one begin the vector part of the quaternion defining the attitude of the base link.
The tasks are, in order of priority:
%The highest priority task is the position of the tail, which is set to a position
%that slowly moves backwards along the south direction. The second task is the position of the head,
%which is set to a fixed position. The third task is the attitude of the base link,
%set have the same orientation as the inertial frame.
\begin{enumerate}
    \item The position of the tail. Moving backwards along the south direction.
    \item The position of the head. Set to a fixed position.
    \item The attitude of the base link. Set to have the same orientation as the inertial frame.
\end{enumerate}
...
\subsection*{Set-Based Velocity-Level Task-Priority Controller}
...
\begin{enumerate}
    \item The position of the tail. Moving downwards and forwards.
    \item The position of the head. Moving downwards and backwards.
    \item The attitude of the base link. Set to have the same orientation as the inertial frame.
\end{enumerate}
